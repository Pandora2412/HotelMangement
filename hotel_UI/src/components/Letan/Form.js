...
const Form = (props) => {
    ...
    useEffect(() => {
        ...
        const getFormAndMenu = async () => {
            try {
                const resForm = await axiosPrivate.get(
                    `/customerbill/${props.form}`,
                    {
                        signal: controller1.signal
                    }
                );
                isMounted && setName(resForm.data.customer.name);
                isMounted && setId(resForm.data.customer.id);
                isMounted && setPhone(resForm.data.customer.phone);
                isMounted && setEmail(resForm.data.customer.email);
                isMounted && setSex(resForm.data.customer.sex);
                isMounted && setBday(resForm.data.customer.bday);
                isMounted && setAccoms(resForm.data.accompanies);
                isMounted && setPay(resForm.data.paid);
                isMounted && setTotal(resForm.data.total);
                isMounted && setServices(resForm.data.services);
                isMounted && setPhongServices(resForm.data.rooms);
        ...
        <div className = "Form">
            <Container fluid style = {{padding: '0'}}>
                ...
                {accoms.map((accom,index) => 
                    <Row key={index} className="justify-content-md-center" style={(index === accoms.length - 1)?{paddingBottom: 25}:{paddingBottom: 0}}>
                ...
                {services.map((s,index) => 
                    <Row key={index} className="justify-content-md-center" style={(index === services.length - 1)?{paddingBottom: 40}:{paddingBottom: 0}}>
                        <Col xl={4} style={{fontWeight: 400}}>{s.name}</Col>
                ...
                <Row className="justify-content-md-center">
                    ...
                    <Col xl={2} style={{fontWeight: 600, fontSize: '120%', textAlign: 'right'}}>{total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</Col>
                </Row>
                <Row className="justify-content-md-center">
                    <Col xl={7}></Col>
                ...
                <Row className="justify-content-md-center" style={{paddingBottom: 25}}>
                    ...
                    <Col xl={2} style={{fontWeight: 600, fontSize: '120%', textAlign: 'right'}}>{(total - pay).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</Col>
                </Row>
                <Row className="justify-content-md-center" style={{paddingBottom: 25}}>
                    <Col xl={8}></Col>
                    ...
                    <Col xl={1}><button className="functionButton" onClick={() => {
                        const updateCustomerBill = async () => {
                            try {
                                ...
                                await axios.post(`/history/Lễ tân ${localStorage.getItem("username")} chỉnh sửa đơn ${props.form}.`);
            ...
            {messageBoxBill && 
                <div className="model">
                    <MessageBox 
                        mess="Xác nhận lưu và xuất đơn" 
                        handleYes={() => {
                            const SaveBillAndDeleteBookdays = async () => {
                                try {
                                    await axiosPrivate.put(
                                        '/customerbill',
                                        {
                                    ...
                                    await axiosPrivate.put(
                                        `/room/${props.form}`,
                                        phongServices.map(room => ({"toa": room.toa, "id": room.id}))
                                    );
                                    ...
                                    await axios.post(`/history/Lễ tân ${localStorage.getItem("username")} xuất đơn ${props.form}.`);

                                    setMessageBoxBill(false);
                                    props.setForm("");
                                } catch (err) {
                                    console.error(err);
                                    <Navigate to="/" state={{ from: location }} replace />
