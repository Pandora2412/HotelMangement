...
const Notice = (props) => {
    ...
    const handleSend = () => {
        ...
        props.notices.push({
            ...
            "time": (now.getDate() > 9?"":"0") + now.getDate() + "/" + (now.getMonth() + 1 > 9?"":"0") + (now.getMonth() + 1) + ", " + (now.getHours() > 9?"":"0") + now.getHours() + ":" + (now.getMinutes() > 9?"":"0") + now.getMinutes(),
    ...
    return (
        <div className="model" style={{background: "rgba(49,49,49,0.8)"}}>
            <div style={{width: '40%', background: 'white', height: '60%', padding: 10, margin: '5% auto'}}>
                    ...
                    {notices?.sort(
                        ...
                        <Col xs={4} md={3} onClick={() => setFullNote(notice)} className="mb-5">
                            <div className="paper">
                                ...
                                <h5 style={{height: '10%', fontFamily: 'Raleway', fontSize: '70%', fontWeight: 400, textAlign: 'right'}}>{notice.time}</h5>
                            </div>
                        </Col>
                    )}
                </Row>   
