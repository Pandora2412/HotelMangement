...
const getRooms = (req, res) => {
    ...
    if (parseInt(req.params.state) === 3) {
        ...
        res.json(data.rooms.slice(a, b).filter(room => (room.bookday.some(day => (new Date(day.checkin).getTime() === new Date(req.params.checkin).getTime()) && (new Date(day.checkout).getTime() === new Date(req.params.checkout).getTime())))));
    }
    else {
        res.json(data.rooms.slice(a, b).filter(room => (room.bookday.every(day => (new Date(day.checkin).getTime() > new Date(req.params.checkout).getTime()) || (new Date(day.checkout).getTime() < new Date(req.params.checkin).getTime())))));
    }
}
