...
const BookRoomTable = (props) => {  
    ...
    const [rowDataBook, setRowDataBook] = useState([]);
        ...
        const getRoom = async () => {
            try {
                const res = await axiosPrivate.get(
                    `/room/2/${props.toa}/${props.checkin}/${props.checkout}`,
                    {
                        signal: controller.signal
                    });
                isMounted && setRowDataBook(sortArray(res.data, props.orderDirection, props.orderColumn));
                isMounted && setDisplay(sortArray(res.data, props.orderDirection, props.orderColumn));
            } catch (err) {
                console.error(err);
    ...
    useEffect(() => {
        setDisplay(
            rowDataBook.filter(phong => {
                const day = phong.bookday.find(book=>((new Date(book.checkin).getTime() === new Date(props.checkin).getTime()) && (new Date(book.checkout).getTime() === new Date(props.checkout).getTime())));
                return ((day.formnum.includes(props.searchString)) || (day.name.includes(props.searchString)) || (day.id.includes(props.searchString)));
            })
        )
    }, [props.searchString])
    ...
    const handleSortRequest = (id) => {
        ...
        setRowDataBook(sortArray(rowDataBook, props.orderDirection, id));
